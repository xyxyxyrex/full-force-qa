...
async function electronSmoke() {
  ...
  try {
    ...
    const overflowColumn=await captureAutomatePage(window.webContents.id,800,1200,true)
    assert.equal(overflowColumn.documentWidth,900)
    assert.equal(overflowColumn.documentHeight,3200)
    const overflowPixels=await require('sharp')(Buffer.from(overflowColumn.dataUrl.split(',')[1],'base64')).metadata()
    assert.equal(overflowPixels.width,800)
    assert.equal(overflowPixels.height,3200)
