...
### Independent Parity accounts (1.4.3 and later)

Parity uses Google sign-in through Supabase Auth, independently of the optional Monday connector. The main process handles PKCE and encrypted sessions; `parity-account-v2` verifies the Supabase user and resolves ownership server-side. Retain `parity-account` and `monday-oauth` for legacy clients. Google credentials belong only in Supabase Auth, and the redirect allow list must include `http://127.0.0.1:51848/account/oauth/callback`. See the [account deployment guide](../docs/independent-accounts-and-tickets.md).

The `parity_accounts`, `parity_user_state`, `parity_projects`, `parity_notes` and `parity_tickets` tables have RLS enabled and grant no direct access to `anon` or `authenticated`. The additive account migration associates a unique Supabase user with an opaque Parity owner or an existing Monday owner. Project capture thumbnails and note attachment bytes remain in Electron's local `userData` directory; only attachment metadata and local URIs are stored in Supabase.

Existing users should sign in with Google, then choose **Restore my existing Monday workspace**. Restoration requires fresh verification of the Monday identity; matching email addresses do not grant ownership. Existing IDs and attachment paths are retained. Disconnecting Monday stops its ticket synchronization without interrupting the Parity account.
